...
class NetworkingLogic: NSObject {
    
    class func findFoursquarePlaces(afNetworkingManager: AFHTTPRequestOperationManager, coordinates: CLLocationCoordinate2D) {
        
        var urlString = "https://api.foursquare.com/v2/venues/explore?client_id=\(foursquareClientId)&client_secret=\(foursquareClientSecret)&ll=\(coordinates.latitude),\(coordinates.longitude)"
    ...
    class func popError(operation: AFHTTPRequestOperation!, error: NSError!) {
